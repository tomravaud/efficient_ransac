...
void Shape::computeInliersIndices(
  ...
  for (auto i : indices_close_to_shape) {
    ...
    if (distance(point) < thresholds_.distance &&
        angle(point) < thresholds_.normal) {
      inliers_indices_.push_back(i);
    }
  }
  // extract the largest connected component
  if (compute_connected_components) extractLargestConnectedComponent(cloud);
}
...
void Shape::getIndicesCloseToShape(
  ...
  for (auto it = octree->leaf_depth_begin(); it != octree->leaf_depth_end();
    ...
    if (pow(dist_voxel_shape - thresholds_.distance, 2) <
        pow(resolution, 2) * 3.0 / 4) {
...
void Shape::getIndicesCloseToShapeSmall(
  ...
  for (auto it = small_octree->leaf_depth_begin();
       it != small_octree->leaf_depth_end(); ++it) {
    Eigen::Vector3f min_pt, max_pt;
    ...
    if (pow(dist_voxel_shape - thresholds_.distance, 2) <
      ...
      it.getLeafContainer().getPointIndices(point_indices_voxel);
      for (auto idx : point_indices_voxel) {
        if (remaining_points[subsampled_indices[idx]])
          indices_close_to_shape.push_back(idx);
      }
    }
  }
}
...
void Shape::computeExpectationScore(
  ...
  expected_score_ = 0;
  std::vector<int> indices_close_to_shape;
  getIndicesCloseToShapeSmall(small_octree, remaining_points,
                              subsampled_indices, indices_close_to_shape);
  for (auto i : indices_close_to_shape) {
  ...
  const int small_cloud_size = std::count_if(
      subsampled_indices.begin(), subsampled_indices.end(),
      [&remaining_points](int index) { return remaining_points[index]; });
  expected_score_ =
      (2 + cloud_size) * (1 + expected_score_) / (2 + small_cloud_size) - 1;
}
